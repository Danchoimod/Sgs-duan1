...
public class NguoiThuePanel extends javax.swing.JPanel implements NguoiThueController {
    ...
    public void setForm(NguoiThue nguoiThue) {
        ...
        try {
            ...
            if (nguoiThue.getTrangThai() != null) {
                String st = nguoiThue.getTrangThai().trim();
                String trangThaiText;
                if (st.equalsIgnoreCase("Hoạt động") || st.equals("1") || st.equalsIgnoreCase("true") || st.equalsIgnoreCase("active")) {
                    trangThaiText = "Hoạt động";
                } else if (st.equalsIgnoreCase("Dừng hoạt động") || st.equals("0") || st.equalsIgnoreCase("inactive")) {
                    trangThaiText = "Dừng hoạt động";
                } else if (st.equalsIgnoreCase("Đã xóa")) {
                    // Không nên hiển thị người dùng đã xóa mềm, nhưng nếu có thì set về Dừng hoạt động
                    trangThaiText = "Dừng hoạt động";
                } else {
                    trangThaiText = "Hoạt động";
                }
                ...
                cboTrangThaiHoatDong.setSelectedIndex(0); // Mặc định Hoạt động
            }
            
            if (nguoiThue.getVaiTro() != null) {
                String vr = nguoiThue.getVaiTro().trim();
                String vaiTroText;
                if (vr.equals("1") || vr.equalsIgnoreCase("true") || vr.equalsIgnoreCase("chủ trọ") || vr.equalsIgnoreCase("chu tro")) {
                    vaiTroText = "Chủ trọ ";
                } else {
                    vaiTroText = "Người thuê";
                }
                cboVaiTro.setSelectedItem(vaiTroText);
            } else {
                cboVaiTro.setSelectedIndex(1); // Mặc định Người thuê
            }
    ...
    public NguoiThue getForm() {
        try {
            ...
            // Lưu đúng dạng chuỗi theo DB: "Hoạt động" hoặc "Dừng hoạt động"
            nguoiThue.setTrangThai(selectedTrangThai != null ? selectedTrangThai.trim() : "Dừng hoạt động");

            String selectedVaiTro = (String) cboVaiTro.getSelectedItem();
            // Lưu vai trò là chuỗi đúng chuẩn theo DB: "Chủ trọ" hoặc "Người thuê"
            if (selectedVaiTro != null) {
                String vr = selectedVaiTro.trim();
                nguoiThue.setVaiTro(vr.equalsIgnoreCase("Chủ trọ") || vr.equalsIgnoreCase("Chủ trọ") || vr.equalsIgnoreCase("Chủ trọ ") ? "Chủ trọ" : "Người thuê");
            } else {
                nguoiThue.setVaiTro("Người thuê");
            }
    ...
    public void fillToTable() {
        try {
            ...
            for (NguoiThue nguoiThue : danhSachNguoiThue) {
                // Hiển thị vai trò dựa trên giá trị text trong DB
                String displayVaiTro;
                if (nguoiThue.getVaiTro() != null && nguoiThue.getVaiTro().trim().equalsIgnoreCase("Chủ trọ")) {
                    displayVaiTro = "Chủ trọ";
                } else {
                    displayVaiTro = "Người thuê";
                }

                // Hiển thị trạng thái nhất quán; 'Đã xóa' sẽ không có trong danh sách findAll
                String displayTrangThai = "Hoạt động".equalsIgnoreCase(String.valueOf(nguoiThue.getTrangThai())) ? "Hoạt động" : "Dừng hoạt động";
                Object[] tableRow = {
    ...
    public void delete() {
        ...
        if(selectRow >= 0) {
            try {
                ...
                // Kiểm tra có hợp đồng hay chưa
                if (dao.hasAnyContract(nguoiThueId)) {
                    XDialog.alert("Không thể xóa người thuê này!\n\n" +
                                 "Lý do: Người thuê đang có hợp đồng hoặc đang ở chung trong hợp đồng.\n" +
                                 "Vui lòng xóa tất cả hợp đồng liên quan trước khi xóa tài khoản người thuê.");
                    return;
                }
...
public void clear() {
        ...
        try {
            ...
            if (XDialog.confirm("Bạn thực sự muốn xóa " + idsToDelete.size() + " mục đã chọn?")) {
                ...
                for (Integer id : idsToDelete) {
                    try {
                        // Chặn xóa nếu còn hợp đồng
                        if (dao.hasAnyContract(id)) {
                            failedCount++;
    ...
    // Validation cho ảnh CCCD/CMND (sẽ quyết định sau khi tính tuổi)
...
// Validate ngày sinh - bắt buộc phải nhập và không được trong tương lai
        ...
        // Tính tuổi chính xác theo ngày/tháng/năm hiện tại
        Calendar dob = Calendar.getInstance();
        dob.setTime(ngaySinh);
        Calendar today = Calendar.getInstance();
        int age = today.get(Calendar.YEAR) - dob.get(Calendar.YEAR);
        // Nếu hôm nay chưa tới sinh nhật năm nay thì trừ 1
        ...
        // Yêu cầu ảnh CCCD nếu tuổi > 14; nếu tuổi <= 14 thì không bắt buộc
        if (age > 14) {
            if (anhMatTruoc == null || anhMatTruoc.trim().isEmpty()) {
                javax.swing.JOptionPane.showMessageDialog(this,
                    "Vui lòng tải lên ảnh mặt trước CCCD/CMND (bắt buộc với người trên 14 tuổi)!",
                    "Lỗi validation",
                    javax.swing.JOptionPane.ERROR_MESSAGE);
                return false;
            }
            if (anhMatSau == null || anhMatSau.trim().isEmpty()) {
                javax.swing.JOptionPane.showMessageDialog(this,
                    "Vui lòng tải lên ảnh mặt sau CCCD/CMND (bắt buộc với người trên 14 tuổi)!",
                    "Lỗi validation",
                    javax.swing.JOptionPane.ERROR_MESSAGE);
                return false;
            }
